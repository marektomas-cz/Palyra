...
fn run_configured_secret_inventory(json: bool) -> Result<()> {
    ...
    for line in secrets_text::render_configured_secret_inventory_lines(&envelope) {
...
fn run_configured_secret_explain(secret_id: &str, json: bool) -> Result<()> {
    ...
    for line in secrets_text::render_configured_secret_explain_lines(&envelope.secret) {
        output::print_text_line(line.as_str())?;
    }
    Ok(())
}
