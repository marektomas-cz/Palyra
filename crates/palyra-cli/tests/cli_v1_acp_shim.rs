...
fn wait_for_tcp_listen(port: u16, daemon: &mut Child, timeout: Duration) -> Result<()> {
    let timeout_at = Instant::now() + timeout;
    let address = SocketAddr::from(([127, 0, 0, 1], port));
    loop {
        if Instant::now() > timeout_at {
            anyhow::bail!("timed out waiting for daemon to accept TCP on port {port}");
        }
        if let Some(status) = daemon.try_wait().context("failed to check daemon status")? {
            let stderr = read_child_stderr(daemon.stderr.take());
            if stderr.is_empty() {
                anyhow::bail!(
                    "palyrad exited before TCP listener became ready on port {port} with status: {status}"
                );
            }
            anyhow::bail!(
                "palyrad exited before TCP listener became ready on port {port} with status: {status}; stderr: {stderr}"
            );
        }
